...
static void GamePhaseSwitch(t_game *const game, const t_game_phase_id phase_id, const zcl::t_v2_i screen_size, const zgl::t_gfx_ticket_mut gfx_ticket, zcl::t_arena *const temp_arena) {
    ...
    switch (phase_id) {
        ...
        case ek_game_phase_id_world: {
            game->phase_data = WorldPhaseInit(gfx_ticket, screen_size, game->phase_arena, temp_arena);
            break;
        }
...
void GameRender(const zgl::t_game_render_func_context &zf_context) {
    ...
    switch (game->phase_id) {
        ...
        case ek_game_phase_id_world: {
            WorldPhaseRender(static_cast<t_world_phase *>(game->phase_data), zf_context.rendering_context, game->assets, zf_context.temp_arena);
            break;
        }
    ...
    zgl::RendererSubmitStr(zf_context.rendering_context, {zcl::ByteStreamGetWritten(&fps_str_bytes_stream)}, *FontGet(game->assets, ek_font_id_roboto_40), {32.0f, static_cast<zcl::t_f32>(zf_context.rendering_context.screen_size.y - 32.0f)}, zcl::k_color_white, zf_context.temp_arena, zcl::k_origin_bottom_left);
